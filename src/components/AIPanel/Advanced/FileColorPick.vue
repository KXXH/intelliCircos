...
<template>
  <Popover v-model:open="open">
    <PopoverTrigger as-child>
      <Circle :class="cn('h-4 w-4', `fill-${value}`, props.class)" :stroke-width="2" />
    </PopoverTrigger>
    <PopoverContent
    ...
    >
      <div grid="~ cols-4">
        <Button
          v-for="color in Color" :key="color" size="icon" variant="ghost"
          class="h-8 w-8 rounded-full transition-transform ease hover:scale-125" @click="click(color)"
        >
          <Circle class="h-4 w-4" :stroke-width="2" :class="`fill-${color}`" />
        </Button>
      </div>
